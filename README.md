...
- Save and load stories. The story autosaves after every continuation, and one backup is generated for each story.
...
The story is saved automatically after every Continue action. It can also be saved manually with the Save button. One backup save is created for each file, which can be accessed with the file name {story name}_backup.json. Saved files and their backups can be found in backend/files. 
